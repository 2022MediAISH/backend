...
def request_call(url, option):
    ...
    ######################################################################################
        if(option == "0"): ##### 0은 acm만
            ...
            #dictionary format
            calc_date, population_ratio, official_title, objective, allocation, enrollment, design_model, masking, intervention_name, title = get_calc_date_acm_only(response), get_population_ratio(response), get_officialTitle(response), get_objective(response), get_allocation_acm_only(response), get_enrollment(response), get_designModel_acm_only(response), get_maksing_acm_only(response), get_interventionName(response), get_title(response)
            ...
            #dictionary format
            calc_date, population_ratio, official_title, objective, allocation, enrollment, design_model, masking, intervention_name, title = get_calc_date_biolinkbert(response), get_population_ratio(response), get_officialTitle(response), get_objective(response), get_allocation_biolink(response), get_enrollment(response), get_designModel_biolink(response), get_maksing_biolink(response), get_interventionName(response), get_title(response)
...
if __name__ == "__main__":
    # sys.argv[1]은 url임
    # sys.argv[2]은 biolink 인지 acm인지 판단
    option = str(sys.argv[2])
    try:
        req = request_call(str(sys.argv[1]), str(sys.argv[2]))
